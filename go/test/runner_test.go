...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("WORLDCUPQUALIFICATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("WORLDCUPQUALIFICATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("WORLDCUPQUALIFICATION_TEST_EXPLAIN"); explain != "" {
