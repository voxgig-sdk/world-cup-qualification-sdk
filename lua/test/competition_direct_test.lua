...
describe("CompetitionDirect", function()
  it("should direct-list-competition", function()
    local setup = competition_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-competition", setup.live and "live" or "unit")
  ...
  it("should direct-load-competition", function()
    local setup = competition_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-competition", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function competition_direct_setup(mockres)
