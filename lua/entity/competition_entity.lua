...
local CompetitionEntity = {}
CompetitionEntity.__index = CompetitionEntity


function CompetitionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CompetitionEntity)
  self._name = "competition"
  self._client = client
...
function CompetitionEntity:get_name()
  return self._name
end


function CompetitionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CompetitionEntity.new(self._client, opts)
end
...
function CompetitionEntity:mark_deleted()
  self._deleted = true
end


function CompetitionEntity:deleted()
  return true == self._deleted
end


function CompetitionEntity:data_set(args)
  if args ~= nil then
...
function CompetitionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CompetitionEntity:match_set(args)
  if args ~= nil then
...
function CompetitionEntity:match_get()
...
function CompetitionEntity:stream(action, args, callopts)
...
---@param reqmatch CompetitionLoadMatch
---@param ctrl? table
---@return Competition
---@return string? err
function CompetitionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch CompetitionListMatch
---@param ctrl? table
---@return Competition[]
---@return string? err
function CompetitionEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function CompetitionEntity:_run_op(ctx, post_done)
  local utility = self._utility
