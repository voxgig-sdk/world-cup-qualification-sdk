...
require("world-cup-qualification_types")
...
local WorldCupQualificationSDK = {}
WorldCupQualificationSDK.__index = WorldCupQualificationSDK
...
WorldCupQualificationSDK._make_feature = _make_feature


function WorldCupQualificationSDK.new(options)
  local self = setmetatable({}, WorldCupQualificationSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function WorldCupQualificationSDK:options_map()
...
function WorldCupQualificationSDK:get_utility()
  return Utility.copy(self._utility)
end


function WorldCupQualificationSDK:get_root_ctx()
  return self._rootctx
end


function WorldCupQualificationSDK:prepare(fetchargs)
...
function WorldCupQualificationSDK:direct(fetchargs)
...
function WorldCupQualificationSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function WorldCupQualificationSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "WorldCupQualificationSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function WorldCupQualificationSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function WorldCupQualificationSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "WorldCupQualificationSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function WorldCupQualificationSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = WorldCupQualificationSDK.new(sdkopts)
  sdk.mode = "test"
...
return WorldCupQualificationSDK
