# world-cup-qualification-mcp

[MCP](https://modelcontextprotocol.io) server exposing the WorldCupQualification SDK as
two agent tools — `world-cup-qualification_list` and `world-cup-qualification_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/world-cup-qualification-mcp)
make build

# 2. Provide credentials via the environment
export WORLD_CUP_QUALIFICATION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user world-cup-qualification \
  -- /absolute/path/to/world-cup-qualification-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./world-cup-qualification-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// world-cup-qualification_list: first page of records
{ "entity": "competition" }
{ "entity": "competition", "query": { } }

// world-cup-qualification_load: one record by id
{ "entity": "competition", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/world-cup-qualification-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user world-cup-qualification \
     -- "$PWD"/dist/*/world-cup-qualification-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `world-cup-qualification_list` and `world-cup-qualification_load` tools now appear
   in new sessions. Ask the agent to *"list competition using world-cup-qualification"*
   and it calls `world-cup-qualification_list` with `{"entity":"competition"}`.
...
```sh
export WORLD_CUP_QUALIFICATION_APIKEY=sk_live_xxx            # API key
export WORLD_CUP_QUALIFICATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `world-cup-qualification_list` tool
...
### Call the `world-cup-qualification_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `world-cup-qualification_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `world-cup-qualification_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `WORLD_CUP_QUALIFICATION_APIKEY` | API key sent with every request. |
| `WORLD_CUP_QUALIFICATION_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

competition | match | standing | team

### Smoke test via HTTP (raw JSON-RPC)

```sh
./world-cup-qualification-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"world-cup-qualification_load","arguments":{"entity":"competition","query":{"id":1}}}}'
```
