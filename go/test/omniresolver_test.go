...
import (
	...
	omni "github.com/voxgig-sdk/world-cup-qualification-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.WorldCupQualificationError); is {
				out := map[string]any{
					"name":    "WorldCupQualificationError",
...
func omniCtx(arg any, client *sdk.WorldCupQualificationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.WorldCupQualificationSDK); is {
			client = live
