...
from worldcupqualification_sdk.worldcupqualification_types import (
    Competition,
    CompetitionLoadMatch,
...
class CompetitionEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CompetitionEntity(self._client, opts)
    ...
    def match_get(self) -> Competition:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Competition:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Competition().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Competition]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Competition().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
