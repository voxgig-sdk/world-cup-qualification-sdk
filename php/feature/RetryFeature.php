...
// WorldCupQualification SDK retry feature
...
class WorldCupQualificationRetryFeature extends WorldCupQualificationBaseFeature
{
    ...
    public function init(WorldCupQualificationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (WorldCupQualificationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(WorldCupQualificationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(WorldCupQualificationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
