# WorldCupQualification TypeScript SDK

The TypeScript SDK for the WorldCupQualification API. Provides a type-safe, entity-oriented interface with full async/await support.
...
```ts
const client = new WorldCupQualificationSDK({ apikey: '...' })
...
```ts
new WorldCupQualificationSDK(options?: {
  apikey?: string
  base?: string
  prefix?: string
  suffix?: string
...
| `tester(testopts?, sdkopts?)` | `WorldCupQualificationSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `WorldCupQualificationSDK.test(testopts?, sdkopts?)` | `WorldCupQualificationSDK` | Create a test-mode client. |
...
├── src/
│   ├── WorldCupQualificationSDK.ts        # Main SDK class
