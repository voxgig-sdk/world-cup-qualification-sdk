// Command WorldCupQualification-cli is a CLI + REPL for the WorldCupQualification SDK,
...
const prompt = "world-cup-qualification"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "competition match standing team"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      WORLD_CUP_QUALIFICATION_APIKEY (api key), WORLD_CUP_QUALIFICATION_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: WORLD_CUP_QUALIFICATION_APIKEY carries the API key and
	// WORLD_CUP_QUALIFICATION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("WORLD_CUP_QUALIFICATION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("WORLD_CUP_QUALIFICATION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewWorldCupQualificationSDK(opts)
