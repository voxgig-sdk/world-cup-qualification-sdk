...
require_relative 'WorldCupQualification_types'
...
class WorldCupQualificationSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = WorldCupQualificationHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = WorldCupQualificationHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, WorldCupQualificationFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    WorldCupQualificationUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = WorldCupQualificationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = WorldCupQualificationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = WorldCupQualificationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => WorldCupQualificationError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue WorldCupQualificationError => err
    ...
    if fetched.is_a?(Hash)
      status = WorldCupQualificationHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = WorldCupQualificationError.new(
        "graphql_error", "WorldCupQualificationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = WorldCupQualificationSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
