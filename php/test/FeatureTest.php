...
// WorldCupQualification SDK feature test
//
...
class FtCtrl extends WorldCupQualificationControl
{
...
class FtClient
{
    ...
    private WorldCupQualificationUtility $utility;

    public function __construct(WorldCupQualificationUtility $utility, array $options)
    {
    ...
    public function get_utility(): WorldCupQualificationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WorldCupQualificationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public WorldCupQualificationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (WorldCupQualificationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WorldCupQualificationConfig::make_feature($name);
    ...
    private static function buildUrl(WorldCupQualificationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WorldCupQualificationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WorldCupQualificationResponse($fetched);
        }
    ...
    private function fail(WorldCupQualificationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WorldCupQualificationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
