...
The PHP SDK for the WorldCupQualification API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Competition(null)->list(null, null);
...
```php
[$result, $err] = $client->Competition(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->WorldCupQualification(null)->load(
    ["id" => "test01"], null
);
