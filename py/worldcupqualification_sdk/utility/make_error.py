...
def make_error_util(ctx, err):
    if ctx is None:
        from worldcupqualification_sdk.core.context import WorldCupQualificationContext
        ctx = WorldCupQualificationContext({}, None)
    ...
    result = ctx.result
    if result is None:
        result = WorldCupQualificationResult({})
    result.ok = False
    ...
    errmsg = ""
    if isinstance(err, WorldCupQualificationError):
        errmsg = err.msg
    ...
    sdk_err = WorldCupQualificationError("", msg, ctx)
