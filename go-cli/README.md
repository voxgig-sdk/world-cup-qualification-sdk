# world-cup-qualification-cli

boru-driven command-line client **and** interactive REPL for the WorldCupQualification
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/world-cup-qualification-cli)
make build

# 2. See usage (words, entities, env vars)
./world-cup-qualification-cli --help

# 3. Provide credentials once, via the environment
export WORLD_CUP_QUALIFICATION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./world-cup-qualification-cli list competition
./world-cup-qualification-cli load 1 competition            # {id:1} shorthand
./world-cup-qualification-cli load '{id:1}' competition       # explicit match map
./world-cup-qualification-cli list match

# 5. Override the API base URL for a single call
WORLD_CUP_QUALIFICATION_BASE=https://api.example.com ./world-cup-qualification-cli list competition

# 6. No arguments -> interactive REPL
./world-cup-qualification-cli
world-cup-qualification> list competition
world-cup-qualification> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/world-cup-qualification-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/world-cup-qualification-cli list competition
   ```
...
```sh
./world-cup-qualification-cli load 1 competition          # scalar shorthand for {id:1}
./world-cup-qualification-cli load '{id:1}' competition     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `world-cup-qualification>`). Each line is
evaluated as its own boru expression:

```text
$ ./world-cup-qualification-cli
world-cup-qualification> list competition
world-cup-qualification> /help
world-cup-qualification> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `competition`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/world-cup-qualification-cli`. |
...
(`./world-cup-qualification-cli <expr>`) and interactively (the REPL), and expressions compose the
