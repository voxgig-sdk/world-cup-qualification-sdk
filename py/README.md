# WorldCupQualification Python SDK

The Python SDK for the WorldCupQualification API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Competition(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.Competition(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.WorldCupQualification(None).load(
    {"id": "test01"}, None
)
...
```
py/
├── worldcupqualification_sdk.py         -- Main SDK module
...
The main module (`worldcupqualification_sdk`) exports the SDK class.
