...
// WorldCupQualification SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class WorldCupQualificationTestFeature extends WorldCupQualificationBaseFeature
{
    ...
    public function init(WorldCupQualificationContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (WorldCupQualificationContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = WorldCupQualificationParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (WorldCupQualificationContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(WorldCupQualificationContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = WorldCupQualificationParam::call($ctx, $k);
