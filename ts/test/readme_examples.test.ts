...
//     (\`new WorldCupQualificationSDK(...)\` / \`WorldCupQualificationSDK.test(...)\` become
//     \`WorldCupQualificationSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'WorldCupQualificationSDK'
...
const TEST_SEED = {"entity":{"competition":{"test01":{"id":"test01"}},"match":{"test01":{"id":"test01"}},"standing":{"test01":{"id":"test01"}},"team":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new WorldCupQualificationSDK(...)\` and any
// \`WorldCupQualificationSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+WorldCupQualificationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/WorldCupQualificationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
