...
class CompetitionEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Competition(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Competition(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = competition_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "competition." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $competition_ref01_data = null;
        if (count($competition_ref01_data_raw) > 0) {
...
function competition_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/competition/CompetitionTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["competition01", "competition02", "competition03"] as $k) {
