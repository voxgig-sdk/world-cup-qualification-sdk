...
The Lua SDK for the WorldCupQualification API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Competition(nil):list(nil, nil)
...
```lua
local result, err = client:Competition(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:WorldCupQualification(nil):load(
  { id = "test01" }, nil
)
