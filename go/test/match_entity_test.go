...
func TestMatchEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Match(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Match(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		matchRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.match")))
...
func matchBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("WORLD_CUP_QUALIFICATION_TEST_MATCH_ENTID")
	...
	idmapResolved := core.ToMapAny(env["WORLD_CUP_QUALIFICATION_TEST_MATCH_ENTID"])
