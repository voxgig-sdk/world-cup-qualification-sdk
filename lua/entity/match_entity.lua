...
local MatchEntity = {}
MatchEntity.__index = MatchEntity


function MatchEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MatchEntity)
  self._name = "match"
  self._client = client
...
function MatchEntity:get_name()
  return self._name
end


function MatchEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MatchEntity.new(self._client, opts)
end
...
function MatchEntity:mark_deleted()
  self._deleted = true
end


function MatchEntity:deleted()
  return true == self._deleted
end


function MatchEntity:data_set(args)
  if args ~= nil then
...
function MatchEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MatchEntity:match_set(args)
  if args ~= nil then
...
function MatchEntity:match_get()
...
function MatchEntity:stream(action, args, callopts)
...
function MatchEntity:_run_op(ctx, post_done)
  local utility = self._utility
