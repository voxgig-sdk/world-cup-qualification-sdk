...
func TestMatchDirect(t *testing.T) {
	t.Run("direct-list-match", func(t *testing.T) {
		setup := matchDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-match", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"competition01"} {
...
type matchDirectSetupResult struct {
...
func matchDirectSetup(mockres any) *matchDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		return &matchDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &matchDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
