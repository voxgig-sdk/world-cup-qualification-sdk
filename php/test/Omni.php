...
//    only - a typed WorldCupQualificationContext there would make every ctx
...
final class WorldCupQualificationOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new WorldCupQualificationContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new WorldCupQualificationSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new WorldCupQualificationResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new WorldCupQualificationError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new WorldCupQualificationResponse($clean['response']);
    ...
    public static function err_from_map($m): ?WorldCupQualificationError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new WorldCupQualificationError(is_string($code) ? $code : '', $msg);
    }
