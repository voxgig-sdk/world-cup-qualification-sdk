...
// WorldCupQualification SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = WorldCupQualificationOmni::makeStructRunner(
    ...
    {
        return WorldCupQualificationOmni::objmodel(WorldCupQualificationOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return WorldCupQualificationOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return WorldCupQualificationOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(WorldCupQualificationOmni::structfix($now, Struct::undef()))
                        !== json_encode(WorldCupQualificationOmni::structfix($val, Struct::undef()))) {
