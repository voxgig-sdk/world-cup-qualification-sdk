...
class CompetitionEntity
  ...
  def make
    opts = @_entopts.dup
    CompetitionEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = WorldCupQualificationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Competition, Hash] the current Competition data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = WorldCupQualificationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Competition fields)
  def match_get
  ...
  #
  # @param reqmatch [CompetitionLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Competition.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Competition, Hash] the loaded Competition; raises WorldCupQualificationError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = WorldCupQualificationHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [CompetitionListMatch, Hash, nil] match filter (any subset of
  #   Competition fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Competition>, Array] the matching Competition items; raises WorldCupQualificationError on failure
