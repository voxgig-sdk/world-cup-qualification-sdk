...
module WorldCupQualificationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = WorldCupQualificationResponse.new({})
    ...
    if fetch_err
      ...
      response = WorldCupQualificationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WorldCupQualificationResponse.new(fetched)
    else
