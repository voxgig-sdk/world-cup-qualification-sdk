...
class MatchDirectTest extends TestCase
{
    public function test_direct_list_match(): void
    {
        $setup = match_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-match", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["competition01"] as $_liveKey) {
...
function match_direct_setup($mockres)
{
