...
(0, node_test_1.describe)('MatchEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['list']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'match.' + op, live))
                return;
        }
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set WORLD_CUP_QUALIFICATION_TEST_MATCH_ENTID JSON to run live');
            return;
        }
        ...
        let match_ref01_data = Object.values(setup.data.existing.match)[0];
        // LIST
        const match_ref01_ent = client.Match();
        const match_ref01_match = {};
        match_ref01_match['competition_id'] = setup.idmap['competition01'];
        const match_ref01_list = (await match_ref01_ent.list(match_ref01_match)).map((e) => e.data());
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/match/MatchTestData.json');
    ...
    let idmap = transform(['match01', 'match02', 'match03', 'competition01', 'competition02', 'competition03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['WORLD_CUP_QUALIFICATION_TEST_MATCH_ENTID'];
...
//# sourceMappingURL=MatchEntity.test.js.map