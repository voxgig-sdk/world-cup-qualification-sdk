...
(0, node_test_1.describe)('CompetitionDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.WorldCupQualificationSDK({
    ...
    (0, node_test_1.test)('direct-load-competition', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-competition', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-competition', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-competition', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.WORLD_CUP_QUALIFICATION_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    const calls = [];
    const env = (0, utility_1.envOverride)({
        'WORLD_CUP_QUALIFICATION_TEST_COMPETITION_ENTID': {},
    ...
    if (live) {
        ...
        const client = new __1.WorldCupQualificationSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.WORLD_CUP_QUALIFICATION_APIKEY,
        }));
        let idmap = env['WORLD_CUP_QUALIFICATION_TEST_COMPETITION_ENTID'];
    ...
    const client = new __1.WorldCupQualificationSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=CompetitionDirect.test.js.map