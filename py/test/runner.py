# WorldCupQualification SDK test runner
...
class WorldCupQualificationTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            WorldCupQualificationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = WorldCupQualificationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = WorldCupQualificationTestRunner.getenv("WORLD_CUP_QUALIFICATION_TEST_LIVE")
        override = WorldCupQualificationTestRunner.getenv("WORLD_CUP_QUALIFICATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = WorldCupQualificationTestRunner.getenv(key)
        ...
        explain = WorldCupQualificationTestRunner.getenv("WORLD_CUP_QUALIFICATION_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if WorldCupQualificationTestRunner._test_control is not None:
            return WorldCupQualificationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                WorldCupQualificationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            WorldCupQualificationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = WorldCupQualificationTestRunner.load_test_control()
...
def load_env_local():
    WorldCupQualificationTestRunner.load_env_local()


def env_override(m):
    return WorldCupQualificationTestRunner.env_override(m)


def entity_data(v):
    return WorldCupQualificationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return WorldCupQualificationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return WorldCupQualificationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return WorldCupQualificationTestRunner.load_test_control()


def live_client_options():
    return WorldCupQualificationTestRunner.live_client_options()


def live_delay_ms():
    return WorldCupQualificationTestRunner.live_delay_ms()
