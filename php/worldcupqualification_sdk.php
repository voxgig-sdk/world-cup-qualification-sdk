...
class WorldCupQualificationSDK
{
    public string $mode;
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = WorldCupQualificationHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = WorldCupQualificationHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!WorldCupQualificationFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, WorldCupQualificationFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return WorldCupQualificationUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = WorldCupQualificationHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = WorldCupQualificationHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = WorldCupQualificationHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new WorldCupQualificationError($op . "_allow",
                "WorldCupQualificationSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = WorldCupQualificationHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new WorldCupQualificationError("graphql_error",
                "WorldCupQualificationSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // Canonical facade: $client->Competition()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->competition()
    // resolves here too.
    public function Competition($data = null)
    {
        require_once __DIR__ . '/entity/competition_entity.php';
        if ($data === null) {
            if ($this->_competition === null) {
                $this->_competition = new CompetitionEntity($this, null);
            }
            return $this->_competition;
        }
        return new CompetitionEntity($this, $data);
    }


    private $_match = null;

    // Canonical facade: $client->Match()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->match()
    // resolves here too.
    public function Match($data = null)
    {
        require_once __DIR__ . '/entity/match_entity.php';
        if ($data === null) {
            if ($this->_match === null) {
                $this->_match = new MatchEntity($this, null);
            }
            return $this->_match;
        }
        return new MatchEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new WorldCupQualificationSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
