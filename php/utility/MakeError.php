...
// WorldCupQualification SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class WorldCupQualificationMakeError
{
    public static function call(?WorldCupQualificationContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new WorldCupQualificationContext([], null);
        }
        $op = $ctx->op ?? new WorldCupQualificationOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new WorldCupQualificationResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof WorldCupQualificationError) ? $err->msg : (string)$err;
        $msg = "WorldCupQualificationSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new WorldCupQualificationError('', $msg, $ctx);
        ...
        if ($err instanceof WorldCupQualificationError) {
